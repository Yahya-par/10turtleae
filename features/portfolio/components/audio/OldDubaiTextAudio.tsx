...
type OldDubaiTextAudioProps = {
  scene: THREE.Object3D;
  nodes: Record<string, THREE.Object3D>;
};

export default function OldDubaiTextAudio({
  scene,
  nodes,
}: OldDubaiTextAudioProps) {
  ...
  useLayoutEffect(() => {
    ...
    const textObject = findSceneObject(
      scene,
      nodes,
      assetNames.text.oldDubai,
      assetNames.text.oldDubaiBlender,
    );

    targetRef.current = textObject;

    if (process.env.NODE_ENV === "development") {
      console.info(
        "[OldDubaiTextAudio] Target:",
  ...
  useFrame(({ camera }) => {
    ...
    if (!wasVisibleRef.current) {
      audioManager.playOldDubaiTextCue();
    }
